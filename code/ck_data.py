...
def g_contacts():
    ...
    !! Does _not_ include those with no label or any  !!
    !! with either the 'dropped' or NoLongerAMember'. !!
    """
    contacts = []
...
#           if ("GaveUpMembership" in labels or
#               "dropped" in labels): continue
...
def ok_set(set_, incl_set={}, excl_set={}):
    ...
    Returns True if a) <set_> & <in_set> overlap
    and b) no overlap of <set_> and <excl_set>.
    """
...
def g_filter(g_dict, include={}, exclude={}):
    """
    <include> set must be in groups, ignored if empty
    <exclude> set must not be in groups
    Returns g_dict without entries that don't include
    any of the groups listed in include.
    Need to exclude "drop outs"
    """
    ret = []
#   n = 0
#   page_limit = 26
    for mapping in g_dict:
        set_ = set(mapping['groups'])
#       print(f"{mapping['last']}: {mapping['groups']}")
#       n += 1
#       if n % page_limit==0: _ = input()
        if not ok_set(mapping['groups'],
                      include, exclude):
#           print(f"excluding {mapping['last']}")
#           n += 1
            continue
#       else:
#           print(f"including {mapping['last']}")
#           n += 1
        ret.append(mapping)
    return ret
...
def g_data_parts(data):
    ...
    (which can use ok_set() to filter its output.)
...
apps_memb_exmem = (3,4,5,6,7,8,9,10,11,14,15,16,
                          18, 26, 27, 28)
...
def compare(g_data, label, which_stati, report):
    ...
    <label> is what gmail calls "group"
    """
