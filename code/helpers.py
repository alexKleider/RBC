...
def date_entry():
    """
    Returns a user provided date in YYYYMMDD format
    or None.
    """
    while True:
        date = input(
            'Enter date (YYYYMMDD, Rtn for today, or "0" to abort): ')
        if date == "":
            return eightdigitdate
        if date == "0":
            return
        if len(date) == 8 and date.isdigit():
            ...
            print('Must enter "0", Rtn or an 8 digit date!')
...
def get_first_friday_of_month(date=None):
    """
    Uses current date unless <date> is provided.
    Returns a datetime.date (just <date> object
    representing the next first Friday of the month.
    """
    if not date:
